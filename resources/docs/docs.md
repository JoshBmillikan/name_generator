...
The `home-routes` handler in the `name-generator.routes.home` namespace
...
The `home-page` function will in turn call the `name-generator.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/name_generator/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `name-generator.handler` namespace:
...
Request middleware functions are located under the `name_generator.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `name-generator.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `name-generator.db.core` in some other namespace.
